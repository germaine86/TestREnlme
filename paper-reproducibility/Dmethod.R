...
#'   $f_i(a_i, \gamma)$. The left-hand side is the response variable
...
#'   parameter in $a_i = A_i\beta + b_i$, mapping each subject-specific
...
#' @param RR_catof Character. Exclusion criterion passed to
#'   \code{\link{MM_base}}. One of \code{"kappa"} (default) or \code{"Thd"}.
#' @param Bhat Optional named numeric vector of pre-computed fixed-effects
...
#'   \describe{
#'     \item{\code{Dhat}}{The estimated scaled covariance matrix
#'       \eqn{\hat D_*}.}
...
#'     \item{\code{data}}{The original \code{data} argument.}
#'     \item{\code{Expr}}{The model formula.}
#'     \item{\code{group}}{The grouping variable name.}
#'     \item{\code{random}}{The random-effects formula list.}
#'     \item{\code{start}}{The starting values used.}
#'     \item{\code{Q}}{A one-sided formula specifying the second-stage design formula.}
#'     \item{\code{MM_base_obj}}{The \code{MM_base} object (MM/MMF only).}
#'   }
#'
#' @examples
...
#' \emph{Biostatistics}, \strong{14}, 144--159.
#'
#' @export
Dmethod <- function(data, Expr, group, random, start,
  ...
  Bhat0 <- Beta_hat(data, Expr, start, verbose = verbose)
  if (is.null(Bhat))Bhat <- Beta_hat(data, Expr2, start2, verbose = verbose)
  
  ## --- Jacobian and predicted values ---------------------------------.
  Dnames <- setNames(gsub("~.*| ","", random),gsub(".*\\+\\s*","", random))
  zp     <- ZandYPred(data,Expr, Expr2, group, random, Bhat0,Bhat, Dnames=Dnames)
  R_list     <- zp$R
  K_list     <- zp$K
  Ypred_list <- zp$Ypred
  ...
  result <- switch(method,
    VLS = .vls_estimator(data, Expr2,R_list, K_list, resid_list, 
                          ids, k, re_names,  start = start2, group = group, 
                           verbose = verbose, is_permuting=is_permuting),
    MM   = {
  ...
  # # update Beta hat and Ypred
  # Beta <- if (!is.null(result$Beta)) result$Beta else Bhat
  # Beta <- Beta[names(start2)]
  # zp   <- ZandYPred(data, Expr2, group, random, Beta, Dnames=Dnames)
  # Ypred_list <- zp$Ypred
  # resid_list <- zp$residuals
  ...
  structure(
    list(
      ...
      Beta         = if(method %in% c("MM", "MMF"))result$Beta  else  Bhat,
      method       = method,
      X            = K_list,
      R            = R_list,
      Ypred        = Ypred_list,
...
.vls_estimator <- function(data, Expr, R_list, K_list, resid_list,
  ...
  #   })
  # 
  # Bhat_GLS <-  Beta_hat(data=data, Expr, start, weights = weights_list, group = group)
  # }else {Bhat_GLS <-  NULL}# This is not required during permutation procedure
  list(Dhat = Dhat, Sigma2 = sigma2)#, Beta=Bhat_GLS
}
...
# ============================================================:
#' @keywords internal
.mm_estimator <- function(mb,re_names,  verbose = 1,is_permuting) { #R_list, k, re_names,  data,
  ...
  #Weighted estimate
  if(!is_permuting){
  Bhat0 <- .Beta_MM_TS(Aai = mb$Aai, k = k, Ti = mb$Ti,
                      start = mb$start, Dhat = Dhat,
                      MM_IDworks = IDworks, random = mb$random)
  
  Bhat <- c(Bhat0, mb$Beta_GB)
  }else {Bhat <-  NULL}
  rownames(Dhat) <- colnames(Dhat) <- names(re_names)
  #Return
  list(Dhat = Dhat, Sigma2 = mb$sigma2, Beta = Bhat)
}
...
# ============================================================:
#' @keywords internal
.mmf_estimator <- function(mb, re_names, verbose = 1, is_permuting) {
